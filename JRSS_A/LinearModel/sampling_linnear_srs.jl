...
@time Threads.@threads for simnum in 1:B
  ...
  #SampleIdx2 = [vcat(StatsBase.sample( (1:length(x[element]))[x[element] .> 0.0], m1, replace=false),
  #                StatsBase.sample( (1:length(x[element]))[x[element] .<= 0.0], m2, replace=false))
  ...
  #M1 = [sum(x[element] .> 0.0) for element in SampleIdx1]
  #M2 = [sum(x[element] .<= 0.0) for element in SampleIdx1]

  #pi2_sampled = [ifelse.(x_sampled[i] .> 0.0, m1 / M1[i], m2 / M2[i])
  ...
  @. pi2mat_sampled_tmp[x_sampled[1] .> 0.0, x_sampled[1] .> 0.0] = pi2mat_sampled_tmp1

  pi2mat_sampled_tmp2 = ones(m2,m2) .* m2 * (m2 - 1) / M2[i] / (M2[i] - 1)
  pi2mat_sampled_tmp2[diagind(pi2mat_sampled_tmp2)] .= m2 / M2[i]
  @. pi2mat_sampled_tmp[x_sampled[1] .<= 0.0, x_sampled[1] .<= 0.0] = pi2mat_sampled_tmp2

  @. pi2mat_sampled_tmp[x_sampled[1] .> 0.0, x_sampled[1] .<= 0.0] = m1 * m2 / M1[i] / M2[i]
  @. pi2mat_sampled_tmp[x_sampled[1] .<= 0.0, x_sampled[1] .> 0.0] = m1 * m2 / M1[i] / M2[i]
  pi2mat_sampled_tmp
  end for i in 1:n]
  =#
  ...
  ## normal approximation w/ v
    ...
    while true
      ...
      resTMP = updatebetamat(ahat, x_sampled, y_sampled, w1_sampled, w2_sampled, pi2mat_sampled, beta_t1, mua_t1, sigma2a_t1) # gaussian quadrature
  ...
  ## normal approximation using profile likelihood
    ...
    while true
      ...
      #resTMP = updatebetamat(ahat, x_sampled, y_sampled, w1_sampled, w2_sampled, beta_t1, mua_t1, sigma2a_t1, K * 5) # MCMC
      resTMP = updatebetamat_S(x_sampled, y_sampled, w1_sampled, w2_sampled, pi2mat_sampled, beta_t1, mua_t1, sigma2a_t1) # gaussian quadrature
      #resTMP = updatebetamat_fast(ahat, x_sampled, y_sampled, w1_sampled, w2_sampled, beta_t1, mua_t1, sigma2a_t1, interval, rtol, K2) # gaussian quadrature
      #resTMP = updatebetamat(ahat, x_sampled, y_sampled, w1_sampled, w2_sampled, beta_t1, mua_t1, sigma2a_t1) # Julia Integral and findroot ftn
      #resTMP = updatebetamat(ahat, x_sampled, y_sampled, w1_sampled, w2_sampled, pi2mat_sampled, beta_t1, mua_t1, sigma2a_t1, 7.0, 1e-5) # gaussian quadrature
      #@show beta_t1[2]
...
CSV.write("summary.txt",  DataFrame(summary_data(theta_res_new3)), header=false)
CSV.write("summary.txt",  DataFrame(summary_data(theta_res)), header=false, append=true)
CSV.write("summary.txt",  DataFrame(summary_data(theta_res_new4)), header=false, append=true)
CSV.write("summary.txt",  DataFrame(summary_data(theta_res_new6)), header=false, append=true)
CSV.write("summary.txt",  DataFrame(summary_data(theta_res_new7)), header=false, append=true)


RES_TOTAL = zeros(24,3)
...
CSV.write("theta_res_new3.csv",  DataFrame(theta_res_new3), header=false)
CSV.write("theta_res.csv",  DataFrame(theta_res), header=false)
CSV.write("theta_res_new4.csv",  DataFrame(theta_res_new4), header=false)
CSV.write("theta_res_new6.csv",  DataFrame(theta_res_new6), header=false)
CSV.write("theta_res_new7.csv",  DataFrame(theta_res_new7), header=false)
CSV.write("error_return.csv",  DataFrame(error_return), header=false)
