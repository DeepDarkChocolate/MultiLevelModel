...
@time Threads.@threads for simnum in 1:B
  ...
  SampleIdx2 = [vcat(StatsBase.sample( (1:length(y[element]))[y[element] .> 0.0], m1, replace=false),
                  StatsBase.sample( (1:length(y[element]))[y[element] .<= 0.0], m2, replace=false))
  ...
  M1 = [sum(y[element] .> 0.0) for element in SampleIdx1]
  M2 = [sum(y[element] .<= 0.0) for element in SampleIdx1]

  pi2_sampled = [ifelse.(y_sampled[i] .> 0.0, m1 / M1[i], m2 / M2[i])
  ...
  @. pi2mat_sampled_tmp[y_sampled[i] .> 0.0, y_sampled[i] .> 0.0] = pi2mat_sampled_tmp1

  pi2mat_sampled_tmp2 = ones(m2,m2) .* m2 * (m2 - 1) / M2[i] / (M2[i] - 1)
  pi2mat_sampled_tmp2[diagind(pi2mat_sampled_tmp2)] .= m2 / M2[i]
  @. pi2mat_sampled_tmp[y_sampled[i] .<= 0.0, y_sampled[i] .<= 0.0] = pi2mat_sampled_tmp2

  @. pi2mat_sampled_tmp[y_sampled[i] .> 0.0, y_sampled[i] .<= 0.0] = m1 * m2 / M1[i] / M2[i]
  @. pi2mat_sampled_tmp[y_sampled[i] .<= 0.0, y_sampled[i] .> 0.0] = m1 * m2 / M1[i] / M2[i]
  pi2mat_sampled_tmp
  end for i in 1:n]
