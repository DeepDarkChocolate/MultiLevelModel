...
function Evis_ftn(xi, yi, w2i, beta_t1_2, sigmaa_t1, mi)
  denom = quadgk(v -> hi_ftn(v, xi, yi, w2i, beta_t1_2, sigmaa_t1, mi), -Inf, Inf)[1]
  num1 = quadgk(v -> v * hi_ftn(v, xi, yi, w2i, beta_t1_2, sigmaa_t1, mi), -Inf, Inf)[1]
...
function updatebetamat_S(x, y, w1, w2, beta_t1_ogn, sigma2a_t1)
  ...
  for i in 1:n
    ...
    w2i = w2[i]
    mi = size(xi)[1]
    Evi1, Evi2 = Evis_ftn(xi, yi, w2i, beta_t1, sigmaa_t1, mi)
    mu_num += w1i * Evi1
    sigma_num += w1i * Evi2
  end
  muhat = mu_num / sum(w1)
  beta_t1[1] = beta_t1[1] + muhat
  @show muhat

  return vcat(beta_t1,  sigma_num / sum(w1) - muhat^2)
end
