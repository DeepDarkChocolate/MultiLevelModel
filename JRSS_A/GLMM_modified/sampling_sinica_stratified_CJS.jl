...
## Simulation start
dirName = string("m", m, "n", n, "beta", beta1, "CJS_Mfixed_new")
...
#=
    ...
    while true
      ...
      ahat = solveahat(x_sampled, y_sampled, w2_sampled, beta_t1, mua_t1, sigma2a_t1)
...
#=
    ## normal approximation using profile likelihood vhat
      beta_t1 = copy(beta_ini)
      mua_t1 = mua_ini
      sigma2a_t1 = sigma2a_ini
      beta_t2 = copy(beta_ini)
      ...
      while true
        ...
        #resTMP = updatebetamat_S(x_sampled, y_sampled, w1_sampled, w2_sampled, beta_t1, mua_t1, sigma2a_t1, 8.0, 1e-2) # Gaussian quadrature(do not use)
        #resTMP = updatebetamat_S_fast(x_sampled, y_sampled, w1_sampled, w2_sampled, beta_t1, mua_t1, sigma2a_t1, interval, rtol, K2) # Gaussian quadrature
        resTMP = updatebetamat_S_fast2(ahat, x_sampled, y_sampled, w1_sampled, w2_sampled, pi2mat_sampled, beta_t1, mua_t1, sigma2a_t1, interval, rtol, K2) # Gaussian quadrature
        # Parameters depend on sigma^2
        # len = sqrt(sigma) * 10, rtol = 1e-4, qd points = len * 2.5???;
        # sigma^2 = 3: length * 2.5 = qd points # len = 17.0, rtol = 1e-4, qd points = 50;
        # sigma^2 = 1: length * 2.5 = qd points # len = 10.0, rtol = 1e-4, qd points = 25;
        ...
        if any([isnan(beta_t2[2]), isnan(mua_t2), isnan(sigma2a_t2)])
          println("simnum", simnum, ", cnt = ", cnt, ": NAN generated(profile likelihood vhat): perturbation on initial parameters")
          beta_t1[2] = beta_ini[2]
          mua_t1 = 2 * mua_ini * rand()
          sigma2a_t1 = 2 * sigma2a_ini * rand()
        elseif  sigma2a_t2 > sigma2a * 20.0
          println("simnum", simnum, ", cnt = ", cnt, ", sigma2a_t2 = ", sigma2a_t2, ": Too large sigma(profile likelihood vhat): perturbation on initial parameters")
          beta_t1[2] = beta_ini[2]
          mua_t1 = 2 * mua_ini * rand()
          sigma2a_t1 = 2 * sigma2a_ini * rand()
        elseif  sigma2a_t2 < sigma2a * 0.05
          println("simnum", simnum, ", cnt = ", cnt, ", sigma2a_t2 = ", sigma2a_t2, ": Too small sigma(profile likelihood vhat): perturbation on initial parameters")
          beta_t1[2] = beta_ini[2]
          mua_t1 = 2 * mua_ini * rand()
          sigma2a_t1 = 2 * sigma2a_ini * rand()
        elseif  cnt > 100
          @warn(": Convergence Failed(profile likelihood vhat)")
          println("simnum", simnum, ", cnt = ", cnt)
          beta_t1[2] = beta_ini[2]
          mua_t1 = mua_ini
          sigma2a_t1 = sigma2a_ini
      ...
      theta_res_new5[simnum,:] = theta_t2
=#
  ## Full-pseudo likelihood
    ...
    while true
      ...
      (beta_t2[2], mua_t2, eta_t2) = foo(x_sampled, y_sampled, w1_sampled, w2_sampled, beta_t1, mua_t1, eta_t1, 1 / sqrt(eta_t1) * 10, rtol, K2)
      if eta_t2 < 0
        eta_t2 = rand()
      end
  ...
  ## Composite(Pairwise)-pseudo likelihood
      ...
      while true
        ...
        (beta_t2[2], mua_t2, eta_t2) = foo2(x_sampled, y_sampled, w1_sampled, pi2mat_sampled, beta_t1, mua_t1, eta_t1, 1 / sqrt(eta_t1) * 10, rtol, K2)
        if eta_t2 < 0
          eta_t2 = rand()
        end
...
function summary_data(theta_res)
  ...
  #Res[:,2] =  round.(vcat(mapslices(Distributions.std, theta_res, dims = 1)...) .* (B-1) ./ B; digits = 4)
  Res[:,3] =   round.(vcat(mapslices(mean, (theta .- transpose(theta_res)).^2, dims = 2)...); digits = 6)
  #Res[:,3] =   round.(vcat(sqrt.(mapslices(mean, (theta .- transpose(theta_res)).^2, dims = 2))...); digits = 4)

  Res = NamedArray(Res)
  NamedArrays.setnames!(Res, ["beta1", "mua", "sigma2a"], 1)
  NamedArrays.setnames!(Res, ["Bias", "Var", "MSE"], 2)
  #NamedArrays.setnames!(Res, ["Bias", "S.E.", "RMSE"], 2)
  return Res
end
...
#println(map(x -> @sprintf("%.4e",x), summary_data(theta_res_new4)))

println(NamedArray(vcat([hcat(summary_data(res)[1,:]...) for res in [theta_res_new3, theta_res, theta_res_new4, theta_res_new5, theta_res_new6, theta_res_new7]]...)))
println(NamedArray(vcat([hcat(summary_data(res)[2,:]...) for res in [theta_res_new3, theta_res, theta_res_new4, theta_res_new5, theta_res_new6, theta_res_new7]]...)))
println(NamedArray(vcat([hcat(summary_data(res)[3,:]...) for res in [theta_res_new3, theta_res, theta_res_new4, theta_res_new5, theta_res_new6, theta_res_new7]]...)))

println(NamedArray(vcat([hcat(summary_data(res)[1,:]...) for res in [theta_res_new4, theta_res_new6, theta_res_new7]]...)))
println(NamedArray(vcat([hcat(summary_data(res)[2,:]...) for res in [theta_res_new4, theta_res_new6, theta_res_new7]]...)))
println(NamedArray(vcat([hcat(summary_data(res)[3,:]...) for res in [theta_res_new4, theta_res_new6, theta_res_new7]]...)))
...
CSV.write(string(dirName, "/summary.txt"),  DataFrame(summary_data(theta_res_new5)), header=false, append=true)
