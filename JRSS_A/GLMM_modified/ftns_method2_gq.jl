  ...
  while true
    ...
    #@time begin
      ...
      if isinf(aihat)
        ...
        beta_num += w1[i] * sum([sum(w2_tildei .* (yi .- Pi_mat[:,k]) .* xi .* pdf(Normal(xquad[k], v(xquad[k], xi, yi, w2i, beta_t1, pi2mati, sigma2a_t1, aihat)), aihat)) for k in 1:K] .* wquad) / denomi
        beta_denom += w1[i] * sum([sum(w2_tildei .* Pi_mat[:,k] .* (1 .- Pi_mat[:,k]) .* xi .^ 2 .* pdf(Normal(xquad[k], v(xquad[k], xi, yi, w2i, beta_t1, pi2mati, sigma2a_t1, aihat)), aihat)) for k in 1:K] .* wquad) / denomi
...
function vi_ftn_v(Pi::Vector{Float64}, w2i, yi, pi2mati)
  ratioi = @. (yi - Pi) * w2i
  p2i = @. 1 / w2i
  varhat = transpose(ratioi) * (1 .- (p2i * transpose(p2i) ./ pi2mati)) * ratioi / sum(@. Pi * (1 - Pi) * w2i)^2
  return sqrt(varhat)
...
function hi_ftn(aihat, ai, xi, yi, w2i, pi2mati, beta_t1_2, mua_t1, sigmaa_t1)
  Pi = @. 1 - 1 / (1 + exp(beta_t1_2 * xi + ai))
  #@show ai
  #@show Pi
  #@show vi_ftn_v(Pi, w2i)
  if isnan(vi_ftn_v(Pi, w2i, yi, pi2mati))
    pdfS = 0.0
  elseif isinf(aihat)
    pdfS = 1 / vi_ftn_v(Pi, w2i, yi, pi2mati)
  else
    pdfS = pdf(Normal(ai, vi_ftn_v(Pi, w2i, yi, pi2mati)), aihat)
  end
  if isinf(pdfS)
    #@warn("Inf generated")
    pdfS = 0.0
...
function Epi_ftn(aihat, xi, yi, w2i, pi2mati, beta_t1_2, mua_t1, sigmaa_t1, m)
  denom = quadgk(v -> hi_ftn(aihat, v, xi, yi, w2i, pi2mati, beta_t1_2, mua_t1, sigmaa_t1), -Inf, Inf)[1]
  return [quadgk(v -> pij_ftn(v, xi[j], beta_t1_2) * hi_ftn(aihat, v, xi, yi, w2i, pi2mati, beta_t1_2, mua_t1, sigmaa_t1), -Inf, Inf)[1] / denom for j in 1:m]
end
...
function Evis_ftn(aihat, xi, yi, w2i, pi2mati, beta_t1_2, mua_t1, sigmaa_t1, m)
  denom = quadgk(v -> hi_ftn(aihat, v, xi, yi, w2i, pi2mati, beta_t1_2, mua_t1, sigmaa_t1), -Inf, Inf)[1]
  num1 = quadgk(v -> v * hi_ftn(aihat, v, xi, yi, w2i, pi2mati, beta_t1_2, mua_t1, sigmaa_t1), -Inf, Inf)[1]
  num2 = quadgk(v -> v^2 * hi_ftn(aihat, v, xi, yi, w2i, pi2mati, beta_t1_2, mua_t1, sigmaa_t1), -Inf, Inf)[1]
  return num1 / denom, num2 / denom
end

function updatebetamat(ahat, x, y, w1, w2, pi2mat, beta_t1_ogn, mua_t1, sigma2a_t1)
  n = size(x)[1]
  mu_num = 0.0
  sigma_num = 0.0
  ...
  while true
    ...
    for i in 1:n
      ...
      m = size(xi)[1]
      #@show i
      beta_num += w1i * sum(w2i .* (yi .- Epi_ftn(aihat, xi, yi, w2i, pi2mati, beta_t1[2], mua_t1, sigmaa_t1, m)) .* xi)
      beta_denom += w1i * sum(w2i .* Epi2_ftn(aihat, xi, yi, w2i, pi2mati, beta_t1[2], mua_t1, sigmaa_t1, m) .* xi .^ 2)
    end
    beta_t2[2] = beta_t1[2] + beta_num / beta_denom
    #@show beta_t2[2]
    if abs(beta_t1[2] - beta_t2[2]) > 1e-5
