...
@time Threads.@threads for simnum in 1:B
  ...
  p = [@. 1 - 1 / (1 + exp(beta0 + beta1 * x[i] + a[i])) for i in 1:M]
  z = [rand(N) for i in 1:M]

  y = [ifelse.(z[i] .< p[i], 1, 0) for i in 1:M]
  ...
  fm1 = fit(MixedModel, fm, DataFrame(X=vcat(x_sampled...), Y=vcat(y_sampled...), fac = string.([i for i in 1:M for j in 1:n])), Binomial())
  ...
  muis, muis2 = EMalg(x_sampled, y_sampled, beta_t, sigma2a_t, M, n, verbose = verbose)

  MSE1 = mean([(muis[i] / n - mean(y[i]))^2 for i in 1:M])
  MSE2 = mean([(muis2[i] / n - mean(y[i]))^2 for i in 1:M])
